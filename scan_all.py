...
def exp_ok(t):
    ...
    return (True,found) if found else (False,'no 1-2yr signal')
...
def gh(b):
    ...
    for j in d.get('jobs',[]):
        ...
        out.append(dict(src='GH',co=b,title=j.get('title','').strip(),loc=j.get('location',{}).get('name',''),
...
def ab(b):
    out=[]
    try: d=get("https://api.ashbyhq.com/posting-api/job-board/%s"%b)
    except Exception: return out
    for j in d.get('jobs',[]):
        ...
        out.append(dict(src='AB',co=b,title=(j.get('title') or '').strip(),loc='; '.join([x for x in locs if x])+(' ['+ctry+']' if ctry else ''),
